...
exports.generateIconsLibrary = () => {
    ...
    srcDirList.forEach((childDir) => {
        ...
        if (stat.isDirectory()) {
            ...
            icons.forEach((icon, i) => {
                ...
                // Camel-case 'iconName'
                const iconVarName = getCamelCaseString(
                    'icon ' +
                        icon
                            .toLowerCase()
                            .replace(fileSuffixToClean, '')
                            .replace(/-|_|\s/ig, ' '),
                );

                // Kebab-case 'icon-name'
                const iconName = icon.toLowerCase().replace(fileSuffixToClean, '').replace(/_|\s/ig, '-');
                if (uniqueIconNames.includes(iconName)) {
                    return;
                }
    ...
    fs.writeFileSync(
        path.join(libraryPath, 'index.ts'),
        `${categoryImportsStr}
...
export {${categoriesList.join(',')}};    
    ...
    fs.writeFileSync(
        path.join(typesPath, 'evo-icon-shape.ts'),
        `export type EvoIconShape = '${uniqueIconNames.join("' | '")}';`,
    );
    fs.writeFileSync(path.join(typesPath, 'index.ts'), `export {EvoIconShape} from './evo-icon-shape';`);

    // Write to public-api.ts
    fs.writeFileSync(
        path.join(DIST_PATH, 'public-api.ts'),
        `
